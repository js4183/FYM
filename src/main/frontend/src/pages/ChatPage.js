...
const ChatPage = ({ socket }) => {
  ...
  useEffect(() => {
    axios.post('/socket/chat/meetlist', {
      nick : nick
    }).then((res)=>{
  ...
  const sendBtn = () => {
    ...
    // DB에 저장하는 axios
    axios.post('/socket/chat/chattingsend', sendMsgInfo)
        .then((res)=>{
  ...
  const curCtRCk = (item) => {
    ...
    axios.post('/socket/chat/meetcontent', {mr_idx : curCtR.mr_idx})
        .then((res)=>{
          console.log(res.data);
          setMsgList(res.data)
        }).catch((err)=>console.log(err));
      ...
      <ChatContainer>
        ...
        <main>
          ...
          <div className='mainContainer' style={{ display: dpChat }}>
            <header>
              <div>
                <h2>{curCtR.mt_title} 방 채팅입니다.</h2>
                <h3>[{curCtR.mt_type}] {curCtR.mt_desc}</h3>
              </div>
